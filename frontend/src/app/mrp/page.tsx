                    ...
                    <div className="mt-6 p-4 bg-green-50 dark:bg-green-950 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                                ...
                                <p className="font-semibold">{runMRPMutation.data.materials_processed}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Planned Orders</p>
                                <p className="font-semibold">{runMRPMutation.data.planned_orders_created}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Purchase Reqs</p>
                                <p className="font-semibold">{runMRPMutation.data.purchase_reqs_created}</p>
                            </div>
                            <div>
                                <p className="text-muted-foreground">Exceptions</p>
                                <p className="font-semibold">{runMRPMutation.data.exceptions.length}</p>
                            </div>
                        </div>
                        {runMRPMutation.data.exceptions.length > 0 && (
                            <div className="mt-3">
                                <p className="text-sm font-medium text-orange-600 mb-2">Exceptions:</p>
                                <div className="max-h-32 overflow-y-auto">
                                    {runMRPMutation.data.exceptions.slice(0, 5).map((exception, index) => (
            ...
            <Card className="shadow-md">
                <Tabs defaultValue="planned-orders">
                    <TabsList className="grid w-full grid-cols-3">
                        <TabsTrigger value="planned-orders" className="flex items-center gap-2">
                            <Package className="h-4 w-4" />
                            Planned Orders ({plannedOrders?.length || 0})
                        </TabsTrigger>
                        <TabsTrigger value="purchase-reqs" className="flex items-center gap-2">
                            ...
                            <div className="rounded-lg border bg-card">
                                    ...
                                    <TableBody>
                                        {plannedOrders?.map((order) => (
                                            <TableRow key={order.id}>
                                                ...
                                                <TableCell>
                                                    <Badge variant={order.status === "PLANNED" ? "default" : "secondary"}>
                                                        {order.status}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell>
                                                    {order.status === "PLANNED" && (
                                                        <Button
                                                            size="sm"
                                                            onClick={() => handleConvertOrder(order.planned_order_id)}
                                ...
                                {(!plannedOrders || plannedOrders.length === 0) && (
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-lg font-semibold mb-2">No Planned Orders</h3>
                                        <p className="text-muted-foreground">
                            ...
                            <div className="rounded-lg border bg-card">
                                ...
                                {(!purchaseReqs || purchaseReqs.length === 0) && (
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-lg font-semibold mb-2">No Purchase Requisitions</h3>
                                        <p className="text-muted-foreground">
