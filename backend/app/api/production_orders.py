...
def create_order(payload: schemas.ProductionOrderCreate, db: Session = Depends(get_db)):
    ...
    try:
        import asyncio
        asyncio.create_task(manager.manager.broadcast(message))
    except Exception:
        pass
    return po
...
def release_order(order_id: str, db: Session = Depends(get_db)):
    ...
    # import asyncio
    # asyncio.create_task(manager.manager.broadcast({"type": "order_released", "order_id": po.orderId, "status": po.status.value}))
...
def confirm_order(order_id: str, payload: schemas.ConfirmationCreate, db: Session = Depends(get_db)):
    ...
    if total_yield >= po.quantity:
        ...
        db.commit()
    import asyncio
    asyncio.create_task(manager.manager.broadcast({"type": "confirmation", "order_id": order_id, "yield_total": total_yield, "order_status": po.status.value}))
